...
namespace ClrTest.Reflection
{
	...
	public class ILReader : IEnumerable<ILInstruction>
	{
		...
		public static string DynamicMethodToString(DynamicMethod method)
		{
		...
		public ILReader(MethodInfo method)
		{
			if (method is DynamicMethod)
			{
				...
				m_resolver = new DynamicScopeTokenResolver(dynamicMethod);
				ILGenerator ilgen = dynamicMethod.GetILGenerator();

				m_fixupSuccess = false;
				try
				{
			...
			{
				m_resolver = new StaticScopeTokenResolver(method);
				MethodBody mb = method.GetMethodBody();
				m_byteArray = mb.GetILAsByteArray();
			}
